...
library(duckdbfs)
library(tidyverse)
source("https://raw.githubusercontent.com/einarhjorleifsson/obus/refs/heads/master/R/dr_tidy.R")
OVERWRITE <- FALSE
# The datras tables ----------------------------------------------------------
hh <-
...
ca <-
  duckdbfs::open_dataset("https://heima.hafro.is/~einarhj/datras_latin/CA.parquet") |>
  dr_tidy() |>
  rename(latin = scientificname_worms)
by.length <-
  hl |>
...
by.length |>
  ...
  write_dataset("/net/hafri.hafro.is/export/home/hafri/einarhj/public_html/datras/by_length.parquet",
                overwrite = OVERWRITE)
by.station |>
  write_dataset("/net/hafri.hafro.is/export/home/hafri/einarhj/public_html/datras/by_station.parquet",
                overwrite = OVERWRITE)
